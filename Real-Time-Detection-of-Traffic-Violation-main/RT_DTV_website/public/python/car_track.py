...
# 安全載入 turn_model 權重：先映射到 CPU、支援新版 weights_only
try:
    _turn_state = torch.load(os.path.join(weight, "turn.pth"),
                             map_location=torch.device('cpu'),
                             weights_only=True)
except TypeError:  # 舊版 torch 沒有 weights_only
    _turn_state = torch.load(os.path.join(weight, "turn.pth"),
                             map_location=torch.device('cpu'))
# 若是 checkpoint 包了一層 'state_dict'，自動取出
if isinstance(_turn_state, dict) and "state_dict" in _turn_state:
    _turn_state = _turn_state["state_dict"]
# 移除可能的 'module.' 前綴（DDP 保存）
_turn_state = {k[7:] if k.startswith("module.") else k: v for k, v in _turn_state.items()}
turn_model.load_state_dict(_turn_state, strict=False)
...
# 安全載入 transformer 權重：先映射到 CPU、支援新版 weights_only
try:
    _trans_state = torch.load(weight_path,
                              map_location=torch.device('cpu'),
                              weights_only=True)
except TypeError:
    _trans_state = torch.load(weight_path,
                              map_location=torch.device('cpu'))
if isinstance(_trans_state, dict) and "state_dict" in _trans_state:
    _trans_state = _trans_state["state_dict"]
_trans_state = {k[7:] if k.startswith("module.") else k: v for k, v in _trans_state.items()}
transformer_model.load_state_dict(_trans_state, strict=False)
